...
import { loginSucess, loginError, registerNewUserSuccess, registerNewUserError } from './actions'
import { AnyAction } from 'redux'
import { Administrator, Airline, Passenger, Credentials, UserActions, PassengerDTO } from './types'
...
function* loginSagas(action: AnyAction){
    try{
        ...
        const loggedUser: Airline | Administrator | Passenger = yield call(() => loginService(credentials))
...
export default function* aggregateSagas() {
    yield all([watchUser()])
}